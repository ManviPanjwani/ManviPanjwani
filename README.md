<div align="center">

# Hi, I'm Manvi Panjwani 👋

### Software Engineer · Cloud & Governance Engineering · Applied AI

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Inter&weight=600&size=20&pause=1000&color=7C3AED&center=true&vCenter=true&width=700&lines=Building+secure%2C+scalable+cloud+systems;Automating+governance+with+policy-as-code;Turning+AI+ideas+into+useful+products)](https://git.io/typing-svg)

[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-7C3AED?style=for-the-badge&logo=vercel&logoColor=white)](https://manvipanjwani.github.io/portfolio/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/manvi-panjwani)
[![Email](https://img.shields.io/badge/Email-Say_Hello-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:manvi.panjwani@gmail.com)
[![Resume](https://img.shields.io/badge/Resume-View-111827?style=for-the-badge&logo=readme&logoColor=white)](./PanjwaniManviResume.pdf)

</div>

## About me

- 🎓 MS in Computer Science, Clark University — December 2025
- 🔐 Former Governance Engineer Intern at **GSK**, building policy-as-code solutions with OPA and CI/CD automation
- 🌐 Former Associate Software Engineer at **Nokia**, working on network service platforms, node discovery, and performance optimization
- 📊 Led a 12-person data science team delivering machine-learning solutions at **Technocolabs**
- 📍 Based in San Francisco, California

## Featured projects

| Project | What it does | Stack |
| --- | --- | --- |
| [SkillBridge AI](https://github.com/ManviPanjwani/SkillBridge-AI) | Analyzes resumes against job descriptions, identifies skill gaps, and generates project recommendations and four-week roadmaps. | Next.js, TypeScript, PostgreSQL, Prisma, OpenAI |
| [Sustainability AI Platform](https://github.com/ManviPanjwani/sustainability-ai-platform) | Measures and forecasts organizational carbon emissions through real-time data ingestion and an AI-powered dashboard. | FastAPI, Next.js, TypeScript, Python |
| [SQL Server → Snowflake](https://github.com/ManviPanjwani/snowconvert-sqlserver-to-snowflake) | Demonstrates schema conversion, stored-procedure rewrites, and verification for a SQL Server-to-Snowflake migration. | T-SQL, Snowflake SQL |
| [PocketPilot](https://github.com/ManviPanjwani/PocketPilot) | A responsive cross-platform application built for web, iOS, and Android. | React Native, Expo, TypeScript, Firebase |

## Technical toolkit

**Languages:** Python · Java · Go · C/C++ · TypeScript · JavaScript · SQL · R

**Cloud, infrastructure & governance:** AWS · Google Cloud · Kubernetes · Docker · Terraform · GitHub Actions · Ansible · OPA/Rego · Linux

**Applications & data:** React · Next.js · FastAPI · PostgreSQL · MySQL · Snowflake · Spark · Redis · Tableau

## Certifications

- Google Cloud Professional Machine Learning Engineer — 2025
- AWS Certified Developer – Associate — 2025
- AWS Certified AI Practitioner — 2025
- Cisco Certified Network Associate (CCNA) — 2020
- IBM Python for Data Science & Machine Learning — 2021

## GitHub activity

<table>
  <tr>
    <td width="50%" align="center">
      <img width="100%" src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=ManviPanjwani&theme=github" alt="Manvi's GitHub statistics" />
    </td>
    <td width="50%" align="center">
      <img width="100%" src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=ManviPanjwani&theme=github" alt="Manvi's top languages by repository" />
    </td>
  </tr>
</table>

<div align="center">
  <img width="100%" src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=ManviPanjwani&theme=github" alt="Manvi's contribution activity" />
</div>

<details>
  <summary><strong>More GitHub insights</strong></summary>
  <br />
  <div align="center">
    <img width="48%" src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=ManviPanjwani&theme=github" alt="Most-used languages by commit" />
    <img width="48%" src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=ManviPanjwani&theme=github&utcOffset=-7" alt="Most productive commit time" />
  </div>
</details>

---

<div align="center">

Open to collaborating on **cloud security, governance automation, and applied AI** projects.

</div>
